...
namespace PixelEngine
{
	public abstract class Game : Display
	{
		...
		private void GameLoop()
		{
			...
			SpriteBatch = new SpriteBatch(this, 10000);
		...
		public Color GetScreenPixel(int x, int y) => defDrawTarget[x, y];
		...
		public bool IsBetween(float val, float min, float max) => val > min && val < max;
		...
		private void UpdateMouse(int x, int y)
		{
			...
			if (!mouseOutOfWindow && (!IsBetween(x / PixWidth, 0, ScreenWidth) || !IsBetween(y / PixHeight, 0, ScreenHeight)))
		...
		#region Drawing
		public void Draw(int x, int y, Color col) => Draw(new Point(x, y), col);
		public void Draw(Point p, Color col)
		{
			if (drawTarget == null)
				return;

			void MakePixel(int a, int b, Color pix)
			{
		...
		public void DrawCircle(Point p, int radius, Color col)
		{
			int x0 = 0;
			int y0 = radius;
			int d = 3 - 2 * radius;
		...
		public void DrawFilledCircle(Point p, int radius, Color col)
		{
			int x0 = 0;
			int y0 = radius;
			int d = 3 - 2 * radius;
		...
		public void DrawEllipse(Point p, int width, int height, Color col)
		{
			if (width == 0 || height == 0)
				return;
		...
		public void DrawFilledEllipse(Point p, int width, int height, Color col)
		{
			if (width == 0 || height == 0)
				return;
		...
		{
			DrawLine(new Point(rect.Left, rect.Top), new Point(rect.Right, rect.Top), col);
			DrawLine(new Point(rect.Right, rect.Top), new Point(rect.Right, rect.Bottom), col);
			DrawLine(new Point(rect.Right, rect.Bottom), new Point(rect.Left, rect.Bottom), col);
			DrawLine(new Point(rect.Left, rect.Bottom), new Point(rect.Left, rect.Top), col);
		}
		public void DrawRect(Point p1, Point p2, Color col)
		{
			...
			DrawRect(new Rectangle(p1, new Point(Math.Abs(p2.X - p1.X - 1), Math.Abs(p2.Y - p1.Y - 1))), col);
		}
		public void DrawFilledRect(Point p, int w, int h, Color col)
		{
			...
			p.X = Clip(p.X, 0, ScreenWidth);
			p.Y = Clip(p.Y, 0, ScreenHeight);

			x2 = Clip(x2, 0, ScreenWidth);
			y2 = Clip(y2, 0, ScreenHeight);

			for (int i = p.X; i < x2; i++)
				for (int j = p.Y; j < y2; j++)
					Draw(i, j, col);
		}
		public void DrawFilledRect(Point p1, Point p2, Color col)
		{
			if (p1.X > p2.X && p1.Y > p2.Y)
			{
				Point temp = p1;
			...
			DrawFilledRect(p1, Math.Abs(p2.X - p1.X - 1), Math.Abs(p2.Y - p1.Y - 1), col);
		}
		...
		public void DrawFilledPolygon(Point[] verts, Color col)
		{
			for (int i = 1; i < verts.Length - 1; i++)
				DrawFilledTriangle(verts[0], verts[i], verts[i + 1], col);
		}
		public void DrawPath(Point[] points, Color col)
		{
		...
		#region Text
		public void DrawText(Point p, string text, Color col, int scale = 1)
		{
