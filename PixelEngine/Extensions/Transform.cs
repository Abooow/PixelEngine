...
namespace PixelEngine.Extensions.Transforms
{
	public class Transform : Extension
	{
		...
		public Transform Translate(float ox, float oy)
		{
		...
		public Transform Copy()
		{
			...
			Array.Copy(matrix, transform.matrix, matrix.Length);
			transform.sourceMat = sourceMat;
			transform.targetMat = targetMat;

			return transform;
		}

		public static void DrawSprite(Sprite spr, Transform transform, Color color)
		{
			if (spr == null)
				return;
			...
			for (float i = sx; i < ex; i++)
			{
				for (float j = sy; j < ey; j++)
				{
					...
					if (color.R == 255 && color.G == 255 && color.B == 255 && color.A == 255)
						...
						Game.Draw((int)i, (int)j, Color.Tint(spr[(int)(ox + 0.5f), (int)(oy + 0.5f)], color));
				}
			}
		}
		#endregion
